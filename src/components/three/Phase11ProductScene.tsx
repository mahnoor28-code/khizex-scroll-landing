...
function ScrollDrivenRig({ progressRef }: ScrollDrivenRigProps): React.ReactElement {
  ...
  useFrame(() => {
    ...
    // `size` comes from R3F and updates automatically whenever the
    // Canvas's container resizes (window resize, orientation change),
    // no manual resize listener needed. On a narrow/portrait viewport,
    // pull the camera back a bit further so the model doesn't get
    // visually cropped by the narrower frame.
    const isPortrait = size.width < size.height;
    camera.position.z = isPortrait ? cameraZ * 1.35 : cameraZ;
  });
...
interface Phase11ProductSceneProps {
  progressRef: React.RefObject<ScrollProgressStore>;
  active: boolean;
}

export default function Phase11ProductScene({
  progressRef,
  active
}: Phase11ProductSceneProps): React.ReactElement {
  return (
    <Canvas
      camera={{ position: [2, 1.5, 3.2], fov: 45 }}
