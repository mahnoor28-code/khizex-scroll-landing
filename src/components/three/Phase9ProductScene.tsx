...
interface Phase9ProductSceneProps {
  progressRef: React.RefObject<ScrollProgressStore>;
  /**
   * When false, the render loop stops entirely — R3F simply never calls
   * requestAnimationFrame for this canvas. Driven by an IntersectionObserver
   * one level up (PinnedShowcase), so a canvas the user has scrolled
   * past doesn't keep burning GPU/battery for content nobody sees.
   */
  active: boolean;
}

export default function Phase9ProductScene({
  progressRef,
  active
}: Phase9ProductSceneProps): React.ReactElement {
  return (
    <Canvas
