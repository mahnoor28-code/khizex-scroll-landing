...
function ScrollDrivenRig({ progressRef }: ScrollDrivenRigProps): React.ReactElement {
  ...
  useFrame(() => {
    ...
    const { rotationY, cameraZ } = getAnimationStateAt(progress);
...
interface Phase5ProductSceneProps {
  progressRef: React.RefObject<ScrollProgressStore>;
}

export default function Phase5ProductScene({ progressRef }: Phase5ProductSceneProps): React.ReactElement {
  return (
